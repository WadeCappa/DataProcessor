...
#pre-condition: L must not be empty
#exp:
...
if __name__ == "__main__":
	...
	for folder in os.listdir(Analysis + "/Markers"): 
		for file in os.listdir(Analysis + "/Markers/" + folder):
			...
			if (len(remain_codes) == 0): #empty list
				am_codes = []
			else:
	...
	new_report = open(Analysis + "/after_filter_SNPreport.txt", 'w')
	...
	while(line): #for each line in the old SNP report
		...
		new_line = tokens[0].replace(".","_") + "\t" + tokens[1] + "\t" + tokens[2] + "\t" + tokens[3] + "\t" + tokens[4]
	...
	new_hit_count = open(Analysis + "/after_filter_HitCounts.txt", 'w')
